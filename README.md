...
it usually takes 12 bytes (!) to represent an RGB pixel.
...
**TL;DR:** For the inpatients: this project allows you to drive **thousands** of WS2812 compatible
pixels with **less than 200 bytes** buffer. [Here is how to use](#how-to-use).

## Theory of Operation
A full frame buffer is not really necessary, because the processor is capable of producing pixel
data faster than the LEDs can consume.
...
running at 160Mhz, which means that for every pixel transferred, the CPU will run 4800 cycles.
...
LEDs will interpret subsequently produced pixels as "starting from the first pixel" -- and that is
the common cause of "flickering": pixels displayed at wrong locations.
...
For example, it is reported that on ESP8266 the worst scheduling jitter observed is produced by the
WiFi sub-system, with magnitudes up to 800us. Therefore, we can use a 1200us budget to comfortably
absorb scheduling jitters. And since each WS2812 pixel takes 30us to transfer, the scheduling jitter
budget translates to a buffer of 40 pixels. So, if the strip is under 40 pixels, we buffer the entire
frame; but for strips more than 40 pixels, regardless of 50, or 300 pixels, we always only need a
40 pixel buffer!
...
So, to drive a 300 pixel strip, we only need a 40 * 3 = 120 bytes buffer, as opposed to 3600 bytes.
...
#### Driver Interface
The driver interface is pretty simple and self-explanatory. Notable common adjustment levers are:
1. `IOConfig::reset_us`: You want to set this value as close to your LED's **lower bound** as possible.
   ...
   * If you have a 200 pixel strip, create a `Renderer` with **less** pixels, say 100 or 50;
...
2. `DriverStart()` parameters, `task_stack` and `task_priority`:
   ...
   - Although the driver (with properly configured reset time, see above) will minimize visual defects
     ...
     demanding tasks is running concurrently, you can customize the rendering task's priority to avoid
     it from being starved, and producing a more smooth transition.

#### Frame Rendering API

First, some concepts:

* A "lightshow" is modeled as a sequence of **transitions**, and each transition composes of
...
4. "Frames" are also abstract concepts, which corresponds to the `Frame` abstract class:
  ...
  - `RGB8BPixel` is an umbrella pixel format for all pixels color data with Red, Green and Blue
    ...
    - Current implementation does not do dynamic conversion. Instead the color component order is
      statically defined, e.g `using RGB8BPixel = GRB888;`. This allows the source code that describes
      colors to be agnostic to the LED's color component ordering.
    - Dynamic color component ordering may be implemented in the future.

### Driver Implementation Notes
For those who are curious about what's under the hood.

#### Interrupt Handling
Two facilities are occupied by the driver: the UART and the FRC1 timer. They both share a single ISR,
which juggles its invocations between the two:
...
If you find the chart too confusing, there are 6 distinct operational modes:
- **Frame transmission mode**: triggered by UART1 TX FIFO empty event. Will receive RGB pixel data
  from the ring buffer, convert that to level sequence, enqueue into TX FIFO; UART1 TX FIFO empty
  is re-armed.
- **Frame interruption mode**: triggered by UART1 TX FIFO empty event, when the frame has not been
  completely received but the ring buffer has no data; Will set HW timer a timeout of
  `time left to deplete the TX FIFO` + `reset time` - *10ns*.

  The idea is that, if there is still no data from the ring buffer when the timer expires, we will
  treat it as a real frame underflow and [drop the rest of the frame's data](#frame-underflow-handling).
  
  Note that data from the ring buffer is always sent and received in whole pixels, so there will
  *not* be cases where only part of the color components of a pixel are sent before frame underflow
  (i.e. the last pixel of a cutoff frame displaying wrong color).
- **End of frame mode**: triggered by UART1 TX FIFO empty event, when a frame has been completely
  received; Will set an internal flag to signal end-of-frame, an set HW timer a timeout of
  `time left to deplete the TX FIFO`.
- **Frame reset mode**: triggered by HW timer, when the "end-of-frame" flag is set. Will re-arm the
  timer with a timeout of `reset time`, and set the "frame-waiting" flag.
- **Frame waiting mode**: triggered by HW timer, when the "frame-waiting" flag is set. Will check if
  the ring buffer has sufficient data for the next frame. If so, *directly* enters
  **frame transmission mode**; otherwise, re-arm the timer with an increasing multiple of
  `reset time` (max 25*`reset time`).
- **Frame drop mode**: triggered by HW timer. Try to receive the rest of the frame's pixel data from
  the ring buffer; If the complete frame has been received, sets "frame-waiting" flag and *directly*
  enters **frame waiting mode**; Otherwise, regardless of how much data received, re-arm the timer
  with a timeout of `reset time`.
...
The driver actually has two ring buffer implementations: one uses the IDF's `BYTEBUF` ring buffer,
and a block-based custom ring buffer. The default and recommended solution is the custom ring buffer.

The IDF ring buffer implementation is more general. Its implementation accounts for multiple senders.
However, the generality reduces its efficiency. OTOH, the custom ring buffer is tailored to the very
purpose of delivering pixels between a renderer task and the ISR. Therefore it has better efficiency.

At 160MHz with very light concurrent loads and sufficiently large buffer size (e.g. >1000us jitter),
there isn't a very significant difference between the two implementations. However, at 80Mhz and
pushing buffer size to the extreme, the custom ring buffer shows better performance and stability.
...
* Rendering a 800 pixel strip, targeting 40fps (the physical limit is around 41fps);
* Using the custom ring buffer resulted in **exactly 40fps**, with zero frame underrun and **zero**
  near-misses (aka. "frame interruption mode"), transition is visually smooth without defects;
* Using the IDF ring buffer resulted in **only about 35fps**, zero frame underrun, but **hundreds**
  of near-misses per second. Very occasionally, I observed a tiny glitch during the transition.
...
rendering can reach about 110fps, because there are cases the renderer can immediately provide the
next frame.) I believe it is sufficient for most general use.
